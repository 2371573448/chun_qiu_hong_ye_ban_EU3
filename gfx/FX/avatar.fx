...
const float3 LIGHT_OFFSET = float3( 250, 500, -500);
...
const float3 LIGHT_OFFSET_FLAG = float3( 250, 500, 500);
const float FLAG_GRAYNESS = 0.1;
...
VS_OUTPUT SkinnedAvatarVS(const VS_INPUT v )
{
	...
	float3 LightPosition = float3( CameraPosition.x + LIGHT_OFFSET.x, LIGHT_OFFSET.y, CameraPosition.z + LIGHT_OFFSET.z );
	Out.LightDirection = -normalize(skinnedPosition - LightPosition );
...
VS_OUTPUT StaticAvatarVS(const VS_INPUT_STATIC v )
{
	...
	float3 LightPosition = float3( CameraPosition.x + LIGHT_OFFSET.x, LIGHT_OFFSET.y, CameraPosition.z + LIGHT_OFFSET.z );
	Out.LightDirection = -normalize( Out.vPosition.xyz - LightPosition );
	Out.EyeVec = -normalize(Out.vPosition - (CameraPosition) );
...
float4 SkinnedAvatarPSFlag( VS_OUTPUT In ) : COLOR
{
	...
	vColor.rgb = vColor.rgb  * saturate(diffuse + 0.1f);
